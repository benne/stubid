...
The documentation is a site at [stubid.dev](https://stubid.dev), the aliases the US-English
conversion kept for one release are gone, and an endpoint the discovery document has always
advertised answers 501 where it used to answer 404.

## What is published

The same seven packages and the same image as 2026.09.2. Three tags point at this release:
...
Pin `2026.09.3` in anything that asserts on bytes.
...
build says 2026.09.3. That is the second release where the two are apart, for the reason the
first one gave: a profile's version answers which recording of the broker is served, and a
recording cannot ship before the release that carries it.

One thing the emulator answers under `/op` did change, and it has a section of its own below:
`/op/connect/ciba` returns 501 where it returned 404. Every other route answers exactly what
2026.09.2 answered — the claims, the errors, the key set and the discovery document are
untouched. The `id_token_hint` divergence is now reported by `GET /_stubid/v1/fidelity`, which
changes what the ledger says about the emulator rather than what the emulator does.
...
Every pull request builds the site with `--warningsAsErrors` and deploys nothing, which makes
docfx the first thing in this repository that has ever resolved a link. It checks both files and anchors, and it found three links that
