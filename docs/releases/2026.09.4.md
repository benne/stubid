...
A running instance will now tell you which recording of the broker it is serving, a route on its
way out says so to callers that are not a .NET compiler, and the control API's JSON field names
are written down and held to a file, the same way the .NET surface now is.

## What is published

The same seven packages and the same image as 2026.09.3, with one listing change: `StubId.Server`
no longer carries package tags or the project readme, which is described below. Three tags point
at this release:
...
Pin `2026.09.4` in anything that asserts on bytes.

## The broker's bytes have moved once

No sitting has been recorded since 2026.09.1, so the profile still names that version while the
build says 2026.09.4.

One claim the emulator answers under `/op` did change, and it has a section of its own below:
`mitid.geo_ip_distance_km` is emitted as `0` where it read `8396`, in the userinfo response and in
the transaction token. That value is derived from the client's connection rather than from
anything the broker decides, which is why it is going; but a suite pinned to 2026.09.3 that
asserts on either payload will see it. Every other route answers exactly what 2026.09.3
answered — the claims, the errors, the key set and the discovery document are untouched.
...
Four pages in the broker reference link to them and now do so by full URL, which is what two
guides already do for the compose recipe under `samples/`. Anything bookmarked under
...
Eight members across three packages were recorded as promising a value they can return null
for — seven of them returns, and one a parameter.
...
reason for the scheme, not a description of anything that has happened: across four releases the
...
broker is being reproduced. Until now neither could be read off a running instance: the build
version comes from outside it — the image tag it was started from, or the package a suite
references — and it still does.
...
A deprecated route now carries the field that says it is deprecated, plus a link to where the
decision is written:
...
The .NET surface got a baseline earlier in this same release. The JSON one had nothing, and that gap was
...
own records. Renaming a property that no test read from the bytes renamed both sides at once and the suite
stayed green — while a suite written in Node, Spring or anything else reading the bytes broke. A
handful of names were already pinned as literals by tests that read the response; most were not.
...
**Any routable address fails the build** — in any committed `.cs`, `.json`, `.raw`, `.head`,
`.md` or `.yml` file, rather than only in the recordings. Loopback, the private ranges and the blocks set aside for documentation all pass, so
an example may still name an address. A four-arc object identifier reads exactly like a dotted
quad and no rule separates them, so the one file that decodes the broker's OCSP responses is
excused by name, narrowly, with the reason recorded beside it.
...
**What this changes for a consumer: one value.** No claim was added or removed, no key moved and
no type changed — but `mitid.geo_ip_distance_km` now reads `0` rather than the distance the
recordings carried.
