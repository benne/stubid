...
```csharp
new StubIdBuilder("ghcr.io/benne/stubid:2026.09.2")
```
...
version as numbers and drops the zero, so the package is `2026.9.2` — which is what a bare
`dotnet add package` writes into a project file. Asking for `--version 2026.09.2` resolves the same
